...
- [x] 4. Coupon_System（`app/coupons/`）の実装
  - [x] 4.1 交換券発行ロジック（`issue_coupon`）の実装（JST日付・Redisによる重複チェック・DB保存）
  - [x] 4.2 `POST /api/coupons/issue`エンドポイントの実装
  - [x] 4.3 `GET /api/coupons/my`・`GET /api/coupons/<coupon_id>`エンドポイントの実装
  - [x] 4.4 交換券検証（`POST /api/coupons/<coupon_id>/verify`）の実装（有効期限・未使用確認）
  - [x] 4.5 交換券使用（`POST /api/coupons/<coupon_id>/redeem`）の実装（使用済み更新・使用日時・店舗ID記録）
  - [x] 4.6 Coupon_Systemの単体テスト（`tests/test_coupons.py`）の作成

- [x] 5. WiFi_Auth（`app/wifi/`）の実装
  - [x] 5.1 パターンA（SSID/AP-MAC検証）ロジックの実装
  - [x] 5.2 パターンB（`X-RasPi-AP: 1`ヘッダー・`192.168.4.0/24`サブネット検証）ロジックの実装
  - [x] 5.3 `POST /api/wifi/verify`エンドポイントの実装
  - [x] 5.4 `GET /api/wifi/spots`エンドポイント（管理者向け）の実装
  - [x] 5.5 WiFi_Authの単体テスト（`tests/test_wifi_auth.py`）の作成

- [x] 6. Captive_Portal（`app/portal/`）の実装
  - [x] 6.1 `GET /portal`キャプティブポータルランディングページ（Jinja2テンプレート）の実装
  - [x] 6.2 `GET /portal/redirect`認証後コンテンツページへのリダイレクト処理の実装
  - [x] 6.3 WiFi_Authとの連携（AP識別情報の検証）の実装
