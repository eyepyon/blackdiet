...
- [ ] 1. プロジェクト基盤セットアップ
  - [ ] 1.1 プロジェクト構成の作成（ディレクトリ構造・仮想環境・requirements.txt）
  - [ ] 1.2 Flaskアプリファクトリ（`app/__init__.py`）の実装（Blueprints登録・設定管理・エラーハンドラー）
  - [ ] 1.3 環境変数管理（`.env.example`・`python-dotenv`設定）
  - [ ] 1.4 SQLAlchemy・Flask-Migrate初期設定とDB接続設定
  - [ ] 1.5 Redisセッション（Flask-Session）の初期設定
  - [ ] 1.6 `run.py`エントリーポイントの作成
  - [ ] 1.7 Dockerfileの作成（`python:3.12-slim`ベース・`gunicorn`起動）
  - [ ] 1.8 Docker Compose（ローカル開発用PostgreSQL・Redis）の作成

- [ ] 2. データモデルの実装
  - [ ] 2.1 `User`モデルの実装（`app/models/user.py`）
  - [ ] 2.2 `Spot`モデルの実装（`app/models/spot.py`）
  - [ ] 2.3 `Coupon`モデルの実装（`app/models/coupon.py`、`unique_daily_spot`制約含む）
  - [ ] 2.4 `Session`モデルの実装（`app/models/session.py`）
  - [ ] 2.5 `PartnerApplication`モデルの実装（`app/models/partner_application.py`）
  - [ ] 2.6 `AdminUser`モデルの実装（`app/models/admin_user.py`）
  - [ ] 2.7 `AdTruckLocation`モデルの実装（`app/models/ad_truck_location.py`）
  - [ ] 2.8 初期マイグレーションファイルの生成（Flask-Migrate）

- [ ] 3. Auth_System（`app/auth/`）の実装
  - [ ] 3.1 LINE Login APIを使ったOAuth 2.0認証フロー（`GET /auth/line/login`・`GET /auth/line/callback`）の実装
